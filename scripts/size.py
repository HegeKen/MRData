    ...
    if branch['branch'] == 'cnmp' or branch['branch'] == 'cnmo' or branch['branch'] == 'cnms':
      for rom in branch['links']:
        if rom['recovery'] =='':
          ...
          url = 'https://bkt-sgp-miui-ota-update-alisgp.oss-ap-southeast-1.aliyuncs.com/'+rom['miui']+'/'+rom['recovery']
          ui_sizes.append(get_file_size(url))
          print("\r"+str(sum(ui_sizes))+" GB                               ",end="")
        if rom['fastboot'] =='':
          k = 0
        else:
          url = 'https://bkt-sgp-miui-ota-update-alisgp.oss-ap-southeast-1.aliyuncs.com/'+rom['miui']+'/'+rom['fastboot']
          ui_sizes.append(get_file_size(url))
          print("\r"+str(sum(ui_sizes))+" GB                               ",end="")
      ...
      if branch['idtag'] == 'Dev' or branch['idtag'] == 'CnOO':
        for rom in devdata["branches"][j]["roms"]:
          ...
          if current['recovery'] =='':
            ...
            url = 'https://bkt-sgp-miui-ota-update-alisgp.oss-ap-southeast-1.aliyuncs.com/'+current['os']+'/'+current['recovery']
          ...
          if current['fastboot'] =='':
            ...
            url = 'https://bkt-sgp-miui-ota-update-alisgp.oss-ap-southeast-1.aliyuncs.com/'+current['os']+'/'+current['fastboot']
            os_sizes.append(get_file_size(url))
            print("\r"+str(sum(os_sizes)+" GB                               "),end="")
