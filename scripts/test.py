...
devices = json.loads(open('public/MRdata/data/devices.json', 'r', encoding='utf-8').read())
for brand in devices:
  for device in devices[brand]:
    devcode = common.stringify(device['code'])
    devdata = common.loadJson(device['code'])
    for branch in devdata['branches']:
      ...
      for rom in reversed(branch['links']):
        ...
        get_sql = f"SELECT * FROM roms WHERE recovery = %s" % (recovery)
