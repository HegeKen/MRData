...
(function (global, factory) {
	...
	function each(target, callback) {
		if (isArrayLike(target)) {
			...
			for (var i$1 = 0; i$1 < keys.length; i$1 += 1) {
				if (
					callback.call(target[keys[i$1]], keys[i$1], target[keys[i$1]]) ===
					false
				) {
					return target;
				}
   ...
   */
	...
	function remove(element, types, func, selector) {
		...
		if (!types) {
			...
			types.split(" ").forEach(function (type) {
				if (type) {
					getHandlers(element, type, func, selector).forEach(
						function (handler) {
							return removeEvent(handler);
						},
					);
   ...
   */
	...
	$.extend = function () {
		...
		return extend.apply(
			void 0,
			[objectN.shift(), objectN.shift()].concat(objectN),
		);
	};
   ...
   */
					...
					if ($(element).is("select[multiple]")) {
						map($(element).find("option"), function (option) {
							return (option.selected =
								computedValue.indexOf(option.value) > -1);
						});
					}
		...
		this.each(function (_, element) {
			...
			$(elements).each(function (_, element) {
				...
				if (
					...
					(["radio", "checkbox"].indexOf(type) === -1 || element.checked)
				) {
	...
	Headroom.prototype.onScroll = function onScroll() {
		...
		this.rafId = window.requestAnimationFrame(function () {
			...
			if (
				currentScrollY > this$1.lastScrollY &&
				currentScrollY >= this$1.options.offset &&
				toleranceExceeded
			) {
				this$1.unpin();
			} else if (
				(currentScrollY < this$1.lastScrollY && toleranceExceeded) ||
				currentScrollY <= this$1.options.offset
			) {
				this$1.pin();
			}
	 ...
	 * @param $item item 元素
	 */
	CollapseAbstract.prototype.transitionEnd = function transitionEnd(
		$content,
		$item,
	) {
		if (this.isOpen($item)) {
	...
	var Collapse = /*@__PURE__*/ (function (CollapseAbstract) {
		...
		Collapse.prototype = Object.create(
			CollapseAbstract && CollapseAbstract.prototype,
		);
			...
			if (data.reInit || data.domLoadedEvent) {
				$(
					'<div class="mdui-textfield-counter">' +
						'<span class="mdui-textfield-counter-inputed"></span> / ' +
						maxLength +
						"</div>",
				).appendTo($textfield);
			}
	...
	Select.prototype.readjustMenu = function readjustMenu() {
		...
		if (this.options.position === "bottom") {
			...
			// transform 的 Y 轴坐标
			transformOriginY =
				this.selectedIndex * itemHeight + itemHeight / 2 + itemMargin + "px";
		}
		// 设置样式
	 ...
	 * @param parameters
	 */
	Tab.prototype.triggerEvent = function triggerEvent(
		name,
		$element,
		parameters,
	) {
		...
		if (this.options.history) {
			...
			if (hash) {
				window.location.hash =
					"" + hash + (hash.indexOf("?") > -1 ? "&" : "?") + "mdui-dialog";
			} else {
