			...
			if (nameIndex === 0 && Array.isArray(computedValue)) {
				...
				if ($(element).is("select[multiple]")) {
					map(
						$(element).find("option"),
						(option) =>
							(option.selected = computedValue.indexOf(option.value) > -1),
					);
				}
...
class Tooltip {
	constructor(selector, options = {}) {
		...
		// 创建 Tooltip HTML
		this.$element = $(
			`<div class="mdui-tooltip" id="${$.guid()}">${this.options.content}</div>`,
		).appendTo(document.body);
...
function layerHTML(index = false) {
	return (
		`<div class="mdui-spinner-layer ${index ? `mdui-spinner-layer-${index}` : ""}">` +
